...
data = np.genfromtxt("./income/train.csv", dtype=None, delimiter=",", names=column_headers, skip_header=1, encoding=None)
...
data = np.genfromtxt("./income/test.csv", dtype=None, delimiter=",", names=column_headers, skip_header=1, encoding=None)

X_test = data[x_labels]
# y_test = data[y_label]
IDs = data['ID']

incomeForest = RandomForest(possibleValues, max_depth=15, criterion='gini_index', unknown= '?', num_features=4, num_trees=100)
incomeForest.train(X_train,y_train)
predictions = {
  'ID': [],
  'Prediction': []
}
for i in range(0,len(IDs)):
   ...
   predictions['Prediction'].append(incomeForest.predict(X_test[i]))
